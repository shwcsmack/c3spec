...
export class ZshInstaller {
  ...
  async getInstallationPath(): Promise<{ path: string; isOhMyZsh: boolean }> {
    ...
    if (isOhMyZsh) {
      ...
      return {
        path: path.join(this.homeDir, '.oh-my-zsh', 'custom', 'completions', '_c3spec'),
        isOhMyZsh: true,
      };
    } else {
      // Standard Zsh completions directory
      return {
        path: path.join(this.homeDir, '.zsh', 'completions', '_c3spec'),
        isOhMyZsh: false,
      };
    }
  }
  ...
  async configureZshrc(completionsDir: string): Promise<boolean> {
    ...
    if (process.env.C3SPEC_NO_AUTO_CONFIG === '1') {
      return false;
    }
  ...
  /**
   * Check if .zshrc has C3Spec configuration markers
   *
  ...
  async uninstall(): Promise<{ success: boolean; message: string }> {
    try {
      ...
      if (zshrcCleaned && !isOhMyZsh) {
        messages.push('Removed C3Spec configuration from ~/.zshrc');
      }
