...
/**
 * Generates Bash completion scripts for the C3Spec CLI.
...
export class BashGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    return `# Bash completion script for C3Spec CLI
...
${helpers}
complete -F _c3spec_completion c3spec
`;
  }
  ...
  private generatePositionalCompletion(positionalType: string | undefined, indent: string): string[] {
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`${indent}_c3spec_complete_changes`);
        break;
      case 'spec-id':
        lines.push(`${indent}_c3spec_complete_specs`);
        break;
      case 'change-or-spec-id':
        lines.push(`${indent}_c3spec_complete_items`);
        break;
      case 'schema-name':
        lines.push(`${indent}_c3spec_complete_schemas`);
        break;
      case 'shell':
