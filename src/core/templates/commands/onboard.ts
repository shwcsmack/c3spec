/**
 * Slash-command template for onboard.
 */
...
export function getOpsxOnboardCommandTemplate(): CommandTemplate {
  return {
    ...
    content: `Guide the user through their first complete C3Spec workflow cycle. This is a teaching experience—you'll do real work in their codebase while explaining each step.
...
> C3Spec CLI is not installed. Install it first, then come back to \`/c3spec:onboard\`.
...
Explore mode (\`/c3spec:explore\`) is for this kind of thinking—investigating before implementing. You can use it anytime you need to think through a problem.
...
**Core workflow:**
 ...
 |-------------------|--------------------------------------------|
 | \`/c3spec:propose\` | Create a change and generate all artifacts |
 | \`/c3spec:explore\` | Think through problems before/during work  |
 | \`/c3spec:apply\`   | Implement tasks from a change              |
 | \`/c3spec:archive\` | Archive a completed change                 |

**Additional commands:**

 | Command            | What it does                                             |
 |--------------------|----------------------------------------------------------|
 | \`/c3spec:new\`      | Start a new change, step through artifacts one at a time |
 | \`/c3spec:continue\` | Continue working on an existing change                   |
 | \`/c3spec:ff\`       | Fast-forward: create all artifacts at once               |
 | \`/c3spec:verify\`   | Verify implementation matches artifacts                  |

---

## What's Next?

Try \`/c3spec:propose\` on something you actually want to build. You've got the rhythm now!
\`\`\`
...
- \`/c3spec:continue <name>\` - Resume artifact creation
- \`/c3spec:apply <name>\` - Jump to implementation (if tasks exist)
...
**Core workflow:**
 ...
 | Command                  | What it does                               |
 |--------------------------|--------------------------------------------|
 | \`/c3spec:propose <name>\` | Create a change and generate all artifacts |
 | \`/c3spec:explore\`        | Think through problems (no code changes)   |
 | \`/c3spec:apply <name>\`   | Implement tasks                            |
 | \`/c3spec:archive <name>\` | Archive when done                          |

**Additional commands:**

 | Command                   | What it does                        |
 |---------------------------|-------------------------------------|
 | \`/c3spec:new <name>\`      | Start a new change, step by step    |
 | \`/c3spec:continue <name>\` | Continue an existing change         |
 | \`/c3spec:ff <name>\`       | Fast-forward: all artifacts at once |
 | \`/c3spec:verify <name>\`   | Verify implementation               |

Try \`/c3spec:propose\` to start your first change.
\`\`\`
