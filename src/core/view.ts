...
export class ViewCommand {
  async execute(targetPath: string = '.'): Promise<void> {
    const c3specDir = path.join(targetPath, 'c3spec');

    if (!fs.existsSync(c3specDir)) {
      console.error(chalk.red('No c3spec directory found'));
      process.exit(1);
    }
    ...
    const changesData = await this.getChangesData(c3specDir);
    const specsData = await this.getSpecsData(c3specDir);
    ...
    console.log('\n' + '═'.repeat(60));
    console.log(chalk.dim(`\nUse ${chalk.white('c3spec list --changes')} or ${chalk.white('c3spec list --specs')} for detailed views`));
  }

  private async getChangesData(c3specDir: string): Promise<{
    draft: Array<{ name: string }>;
    active: Array<{ name: string; progress: { total: number; completed: number } }>;
    completed: Array<{ name: string }>;
  }> {
    const changesDir = path.join(c3specDir, 'changes');
  ...
  private async getSpecsData(c3specDir: string): Promise<Array<{ name: string; requirementCount: number }>> {
    const specsDir = path.join(c3specDir, 'specs');
