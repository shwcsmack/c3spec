...
function getWelcomeText(): string[] {
  return [
    chalk.white.bold('Welcome to C3Spec'),
    chalk.dim('A lightweight spec-driven framework'),
    '',
    chalk.white('This setup will configure:'),
    chalk.dim('  • Agent Skills for AI tools'),
    chalk.dim('  • /c3spec:* slash commands'),
    '',
    chalk.white('Quick start after setup:'),
    `  ${chalk.yellow('/c3spec:new')}      ${chalk.dim('Create a change')}`,
    `  ${chalk.yellow('/c3spec:continue')} ${chalk.dim('Next artifact')}`,
    `  ${chalk.yellow('/c3spec:apply')}    ${chalk.dim('Implement tasks')}`,
    '',
