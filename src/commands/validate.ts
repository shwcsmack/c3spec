...
export class ValidateCommand {
  ...
  private printNonInteractiveHint(): void {
    ...
    console.error('  c3spec validate --all');
    console.error('  c3spec validate --changes');
    console.error('  c3spec validate --specs');
    console.error('  c3spec validate <item-name>');
  ...
  private async validateDirectItem(itemName: string, opts: { typeOverride?: ItemType; strict: boolean; json: boolean }): Promise<void> {
    ...
    if (!opts.typeOverride && isChange && isSpec) {
      ...
      console.error('Pass --type change|spec, or use: c3spec change validate / c3spec spec validate');
      process.exitCode = 1;
      return;
    }
  ...
  private async validateByType(type: ItemType, id: string, opts: { strict: boolean; json: boolean }): Promise<void> {
    ...
    if (type === 'change') {
      const changeDir = path.join(process.cwd(), 'c3spec', 'changes', id);
    ...
    const file = path.join(process.cwd(), 'c3spec', 'specs', id, 'spec.md');
    const start = Date.now();
  ...
  private async runBulkValidation(scope: { changes: boolean; specs: boolean }, opts: { strict: boolean; json: boolean; concurrency?: string; noInteractive?: boolean }): Promise<void> {
    ...
    const concurrency = normalizeConcurrency(opts.concurrency) ?? normalizeConcurrency(process.env.C3SPEC_CONCURRENCY) ?? DEFAULT_CONCURRENCY;
    const validator = new Validator(opts.strict);
