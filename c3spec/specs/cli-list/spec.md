...
- **WHEN** `c3spec list` is executed without flags
- **THEN** scan the `c3spec/changes/` directory for change directories
- **AND** exclude the `archive/` subdirectory from results
- **AND** parse each change's `tasks.md` file to count task completion

#### Scenario: Scanning for specs
- **WHEN** `c3spec list --specs` is executed
- **THEN** scan the `c3spec/specs/` directory for capabilities
...
- **WHEN** `c3spec/changes/` directory doesn't exist
...
This command provides that visibility with minimal effort, following c3spec's philosophy of simplicity and clarity.