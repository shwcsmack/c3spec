...
- **THEN** the system creates `c3spec/changes/add-auth/` directory

#### Scenario: Duplicate change rejected
- **WHEN** `createChange(projectRoot, 'add-auth')` is called and `c3spec/changes/add-auth/` already exists
- **THEN** the system throws an error indicating the change already exists

#### Scenario: Creates parent directories if needed
- **WHEN** `createChange(projectRoot, 'add-auth')` is called and `c3spec/changes/` does not exist
...
- **GIVEN** the command runs from an c3spec workspace root
- **WHEN** the user creates a new change
- **THEN** c3spec SHALL create the change under the workspace planning path
- **AND** it SHALL not create the change under a linked repo's `c3spec/changes/` directory
...
- **GIVEN** the command runs from a subdirectory of an c3spec workspace planning home
- **WHEN** the user creates a new change
- **THEN** c3spec SHALL resolve the current workspace as the planning home
...
- **THEN** c3spec SHALL preserve repo-local change creation behavior for that location
...
- **GIVEN** the command runs outside an c3spec workspace
- **WHEN** the user creates a new change in a repo-local c3spec project
- **THEN** c3spec SHALL continue to create the change under `c3spec/changes/`
...
- **THEN** c3spec SHALL reject those invalid affected areas
...
- **THEN** c3spec SHALL create the workspace change
