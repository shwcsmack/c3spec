...
- **WHEN** user runs `c3spec schema which my-workflow` and schema exists in `c3spec/schemas/my-workflow/`
...
- **WHEN** user runs `c3spec schema which my-workflow` and schema exists only in user data directory
...
- **WHEN** user runs `c3spec schema which spec-driven` and both project and package have `spec-driven`
...
- **WHEN** user runs `c3spec schema which spec-driven --json`
...
- **WHEN** user runs `c3spec schema which --all`
...
- **WHEN** user runs `c3spec schema which --all --json`
