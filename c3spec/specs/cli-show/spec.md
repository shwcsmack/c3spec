...
- **WHEN** executing `c3spec show` without arguments
...
- **WHEN** executing `c3spec show` without arguments
- **THEN** do not prompt
- **AND** print a helpful hint with examples for `c3spec show <item>` or `c3spec change/spec show`
- **AND** exit with code 1

#### Scenario: Direct item display

- **WHEN** executing `c3spec show <item-name>`
...
- **AND** if it matches both, print an ambiguity error and suggest `--type change|spec` or using `c3spec change show`/`c3spec spec show`
- **AND** if it matches neither, print not-found with nearest-match suggestions

#### Scenario: Explicit type override

- **WHEN** executing `c3spec show --type change <item>`
...
- **WHEN** executing `c3spec show <item> --json`
...
- **WHEN** showing a change with `c3spec show <change-name> --deltas-only`
...
- **WHEN** showing a spec with `c3spec show <spec-id> --requirements`
