...
- **THEN** show error "No deltas found" with guidance:
  ...
  - Remind authors that files must live under `c3spec/changes/{id}/specs/<capability>/spec.md`
  - Include an explicit note: "Spec delta files cannot start with titles before the operation headers"
  - Suggest running `c3spec change show {id} --json --deltas-only` for debugging
...
- **THEN** include expected header names and a minimal skeleton:
  ...
  - Mention the quick-reference section in `c3spec/AGENTS.md` as the authoritative template
...
- **WHEN** executing `c3spec validate` without arguments
...
- **WHEN** executing `c3spec validate <item-name>`
...
- **WHEN** executing `c3spec validate --all`
- **THEN** validate all changes in c3spec/changes/ (excluding archive)
- **AND** validate all specs in c3spec/specs/
...
- **THEN** include all change proposals under `c3spec/changes/`
- **AND** exclude the `c3spec/changes/archive/` directory

- **WHEN** validating with `--specs`
- **THEN** include all specs that have a `spec.md` under `c3spec/specs/<id>/spec.md`

#### Scenario: Validate all changes

- **WHEN** executing `c3spec validate --changes`
- **THEN** validate all changes in c3spec/changes/ (excluding archive)
...
- **WHEN** executing `c3spec validate --specs`
- **THEN** validate all specs in c3spec/specs/
...
- **WHEN** executing `c3spec validate --all --strict`
...
- **WHEN** executing `c3spec validate --all --json`
...
- **WHEN** executing `c3spec validate --all --json` (or `--changes` / `--specs`)
...
- **AND** suggest passing `--type change` or `--type spec`, or using `c3spec change validate` / `c3spec spec validate`
...
- **WHEN** executing `c3spec validate --type change <item>`
...
- **WHEN** `c3spec validate` is executed with `--no-interactive` or with environment `OPEN_SPEC_INTERACTIVE=0`
...
- **WHEN** running `c3spec validate <change-id>`
