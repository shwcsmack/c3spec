...
- **WHEN** `c3spec/config.yml` exists and `c3spec/config.yaml` does not exist
- **THEN** system reads from `c3spec/config.yml`
...
- **WHEN** config file has syntax errors and user runs `c3spec new change`
