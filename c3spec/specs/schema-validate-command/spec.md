...
- **WHEN** user runs `c3spec schema validate my-workflow`
...
- **WHEN** user runs `c3spec schema validate` without a name
- **THEN** system validates all schemas in `c3spec/schemas/`
- **AND** displays results for each schema
- **AND** exits with non-zero code if any schema is invalid

#### Scenario: Schema not found
- **WHEN** user runs `c3spec schema validate nonexistent`
...
- **WHEN** user runs `c3spec schema validate my-workflow` and `schema.yaml` has syntax errors
...
- **WHEN** user runs `c3spec schema validate my-workflow --json` and schema is valid
- **THEN** system outputs JSON with `valid: true`, `name`, and `path` fields

#### Scenario: JSON output for invalid schema
- **WHEN** user runs `c3spec schema validate my-workflow --json` and schema has errors
...
- **WHEN** user runs `c3spec schema validate my-workflow --verbose`
