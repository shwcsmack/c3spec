...
- **WHEN** user executes `c3spec completion --help`
...
- **WHEN** user executes `c3spec completion generate zsh`
...
- **WHEN** user executes `c3spec completion generate bash`
...
- **WHEN** user executes `c3spec completion generate fish`
- **THEN** output a complete Fish completion script to stdout
- **AND** use `complete -c c3spec` with conditions
...
- **WHEN** user executes `c3spec completion generate powershell`
- **THEN** output a complete PowerShell completion script to stdout
- **AND** use `Register-ArgumentCompleter -CommandName c3spec`
...
- **THEN** discover active changes from `c3spec/changes/` directory
- **AND** exclude archived changes in `c3spec/changes/archive/`
- **AND** return change IDs as completion suggestions
- **AND** only provide suggestions when inside an c3spec-enabled project

#### Scenario: Completing spec IDs

- **WHEN** completing arguments for commands that accept spec names (show, validate)
- **THEN** discover specs from `c3spec/specs/` directory
...
- **WHEN** user requests completions outside an c3spec project
...
- **WHEN** user executes `c3spec completion install zsh`
...
- **AND** write completion script to `~/.oh-my-zsh/custom/completions/_c3spec`
...
- **WHEN** user executes `c3spec completion install zsh` and Oh My Zsh is not detected
- **THEN** create completions directory at `~/.zsh/completions/` if it doesn't exist
- **AND** write completion script to `~/.zsh/completions/_c3spec`
...
- **WHEN** user executes `c3spec completion install bash`
- **THEN** detect if bash-completion is installed by checking for `/usr/share/bash-completion` or `/etc/bash_completion.d`
- **AND** if bash-completion is available, write to `/etc/bash_completion.d/c3spec` (with sudo) or `~/.local/share/bash-completion/completions/c3spec`
- **AND** if bash-completion is not available, write to `~/.bash_completion.d/c3spec` and source it from `~/.bashrc`
...
- **WHEN** user executes `c3spec completion install fish`
- **THEN** create Fish completions directory at `~/.config/fish/completions/` if it doesn't exist
- **AND** write completion script to `~/.config/fish/completions/c3spec.fish`
...
- **WHEN** user executes `c3spec completion install powershell`
...
- **WHEN** user executes `c3spec completion install` without specifying a shell
...
- **WHEN** user executes `c3spec completion uninstall zsh`
...
- **AND** if user confirms, remove `~/.oh-my-zsh/custom/completions/_c3spec` if Oh My Zsh is detected
- **AND** remove `~/.zsh/completions/_c3spec` if standard Zsh setup is detected
- **AND** remove fpath modifications from `~/.zshrc` using marker-based removal
- **AND** display success message

#### Scenario: Uninstalling Bash completion

- **WHEN** user executes `c3spec completion uninstall bash`
...
- **WHEN** user executes `c3spec completion uninstall fish`
- **THEN** prompt for confirmation (unless `--yes` flag provided)
- **AND** if user confirms, remove `~/.config/fish/completions/c3spec.fish`
- **AND** display success message (no config file modification needed)

#### Scenario: Uninstalling PowerShell completion

- **WHEN** user executes `c3spec completion uninstall powershell`
- **THEN** prompt for confirmation (unless `--yes` flag provided)
...
- **WHEN** user executes `c3spec completion uninstall` without specifying a shell
...
- **AND** implement methods:
  ...
  - `isc3specProject(): boolean` - Checks if current directory is c3spec-enabled
...
- **WHEN** `c3spec completion install` cannot detect current shell
...
- **AND** allow redirection to files: `c3spec completion generate zsh > /path/to/_c3spec`
