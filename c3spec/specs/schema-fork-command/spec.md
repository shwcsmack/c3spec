...
- **WHEN** user runs `c3spec schema fork spec-driven my-custom`
- **THEN** system locates `spec-driven` schema using resolution order (project → user → package)
- **AND** copies all files to `c3spec/schemas/my-custom/`
...
- **WHEN** user runs `c3spec schema fork spec-driven` without specifying a name
- **THEN** system copies to `c3spec/schemas/spec-driven-custom/`
- **AND** updates `name` field in `schema.yaml` to `spec-driven-custom`

#### Scenario: Source schema not found
- **WHEN** user runs `c3spec schema fork nonexistent`
...
- **WHEN** user runs `c3spec schema fork spec-driven my-custom` and `c3spec/schemas/my-custom/` exists
...
- **WHEN** user runs `c3spec schema fork spec-driven my-custom --force` and destination exists
...
- **WHEN** user runs `c3spec schema fork spec-driven my-custom` in interactive mode and destination exists
...
- **WHEN** user runs `c3spec schema fork spec-driven my-custom --json`
...
- **WHEN** user runs `c3spec schema fork spec-driven --json`
