...
- **GIVEN** the command runs from an c3spec workspace
...
- **WHEN** c3spec reports or creates planning artifacts for that change
...
- **THEN** c3spec SHALL capture the product goal at the workspace change level
...
- **WHEN** c3spec creates a workspace-level change
...
- **THEN** c3spec SHALL validate those area names against the workspace links
...
- **THEN** c3spec SHALL allow the shared plan to exist before all affected areas are finalized
...
- **WHEN** c3spec reports or creates workspace-scoped specs
...
- **THEN** c3spec SHALL require an explicit implementation workflow with a selected affected area
