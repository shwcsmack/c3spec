...
<!--
DIRECT MEMORY CAPTURE (replaces deferred promotion):
When marking a candidate as promoted (changing - [ ] to - [x]):
1. Create the memory file immediately at openspec/memory/<category>/<slug>.md
2. Add a line to openspec/memory/MEMORY.md under the correct category
3. Commit: "docs(memory): add <slug> from <change-name> retro"

Categories: bug-patterns/ | design-decisions/ | workflow/ | constraints/
Do not leave - [x] candidates without a corresponding memory file — promotion = file created.

Also: grep prior retros for unchecked candidates before writing new ones:
  grep -A 5 '^- \[ \]' openspec/changes/archive/*/retrospective.md
Carry forward, promote, or mark stale as appropriate.
-->
