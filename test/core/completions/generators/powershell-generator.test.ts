...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for C3Spec CLI');
			expect(script).toContain('$c3specCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for c3spec command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize C3Spec',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName c3spec');
			expect(script).toContain('-ScriptBlock $c3specCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-C3SpecChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-C3SpecSpecs');
		});
		...
		it('should handle positional arguments for schema names', () => {
			...
			expect(script).toContain('Get-C3SpecSchemas');
			expect(script).toContain('c3spec __complete schemas 2>$null');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-C3SpecChanges');
			expect(script).toContain('c3spec __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-C3SpecSpecs');
			expect(script).toContain('c3spec __complete specs 2>$null');
		});
