...
describe('instruction-loader', () => {
  ...
  describe('loadChangeContext', () => {
    ...
    beforeEach(() => {
      tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'c3spec-test-'));
    });
    ...
    it('should detect completed artifacts', () => {
      ...
      const changeDir = path.join(tempDir, 'c3spec', 'changes', 'my-change');
    ...
    it('should auto-detect schema from .c3spec.yaml metadata', () => {
      // Create change directory with metadata file
      const changeDir = path.join(tempDir, 'c3spec', 'changes', 'my-change');
      fs.mkdirSync(changeDir, { recursive: true });
      fs.writeFileSync(path.join(changeDir, '.c3spec.yaml'), 'schema: spec-driven\ncreated: "2025-01-05"\n');
...
`
      ...
      it('should deduplicate validation warnings within session', () => {
        ...
        const freshTempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'c3spec-test-'));

        try {
          // Create project config with a uniquely named invalid artifact ID
          const configDir = path.join(freshTempDir, 'c3spec');
          fs.mkdirSync(configDir, { recursive: true });
          fs.writeFileSync(
            path.join(configDir, 'config.yaml'),
            `schema: spec-driven
