...
async function runConfigCommand(args: string[]): Promise<void> {
  ...
  await program.parseAsync(['node', 'c3spec', 'config', ...args]);
}
...
describe('deriveProfileFromWorkflowSelection', () => {
  ...
  it('returns custom when selection is a superset of core workflows', async () => {
    const { deriveProfileFromWorkflowSelection } = await import('../../src/commands/config.js');
    expect(deriveProfileFromWorkflowSelection(['explore', 'sync', 'archive', 'verify'])).toBe('custom');
  });

  it('returns core when selection has exactly core workflows in different order', async () => {
    const { deriveProfileFromWorkflowSelection } = await import('../../src/commands/config.js');
    expect(deriveProfileFromWorkflowSelection(['archive', 'sync', 'explore'])).toBe('core');
  });
});
...
describe('config profile interactive flow', () => {
  ...
  function setupDriftedProjectArtifacts(projectDir: string): void {
    fs.mkdirSync(path.join(projectDir, 'c3spec'), { recursive: true });
    const exploreSkillPath = path.join(projectDir, '.claude', 'skills', 'c3spec-explore', 'SKILL.md');
    fs.mkdirSync(path.dirname(exploreSkillPath), { recursive: true });
    fs.writeFileSync(exploreSkillPath, 'name: c3spec-explore\n', 'utf-8');
  }
  ...
  function setupSyncedCoreBothArtifacts(projectDir: string): void {
    ...
    const coreCommands = ['explore', 'sync', 'archive'];
    for (const commandId of coreCommands) {
      const commandPath = path.join(projectDir, '.claude', 'commands', 'c3spec', `${commandId}.md`);
  ...
  function addExtraVerifyWorkflowArtifacts(projectDir: string): void {
    const verifySkillPath = path.join(projectDir, '.claude', 'skills', 'c3spec-verify-change', 'SKILL.md');
    fs.mkdirSync(path.dirname(verifySkillPath), { recursive: true });
    fs.writeFileSync(verifySkillPath, 'name: c3spec-verify-change\n', 'utf-8');

    const verifyCommandPath = path.join(projectDir, '.claude', 'commands', 'c3spec', 'verify.md');
  ...
  function setupWorkspaceState(
    ...
    const metadataDir = path.join(workspaceRoot, '.c3spec-workspace');
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `c3spec-config-profile-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('delivery-only action should not invoke workflow checkbox prompt', async () => {
    ...
    saveGlobalConfig({ featureFlags: {}, profile: 'core', delivery: 'both', workflows: ['explore', 'sync', 'archive'] });
  ...
  it('workflows-only action should not invoke delivery prompt', async () => {
    ...
    checkbox.mockResolvedValueOnce(['explore', 'sync']);
    ...
    const proposeChoice = checkboxCall.choices.find((choice: { value: string }) => choice.value === 'explore');
    ...
    expect(getGlobalConfig().workflows).toEqual(['explore', 'sync']);
  });
  ...
  it('workflow picker should use friendly names with descriptions', async () => {
    ...
    saveGlobalConfig({ featureFlags: {}, profile: 'core', delivery: 'both', workflows: ['explore', 'sync', 'archive'] });
    select.mockResolvedValueOnce('workflows');
    checkbox.mockResolvedValueOnce(['explore', 'sync', 'archive']);
  ...
  it('keep action should report no config changes when values are unchanged', async () => {
    const { saveGlobalConfig } = await import('../../src/core/global-config.js');
    const { select } = await getPromptMocks();
  ...
  it('effective no-op after prompts should not force apply prompts', async () => {
    const { saveGlobalConfig } = await import('../../src/core/global-config.js');
    const { select, confirm } = await getPromptMocks();
  ...
  it('keep action with extra project workflows still reports no global config changes', async () => {
    const { saveGlobalConfig } = await import('../../src/core/global-config.js');
    const { select } = await getPromptMocks();
  ...
  it('changed config should ask to apply to the current workspace and print workspace guidance when declined', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('Config updated. Run `c3spec workspace update` to apply it to workspace-local skills.');
  });
  ...
  it('confirmed workspace apply should run workspace update instead of repo-local update', async () => {
    ...
    expect(execSync).toHaveBeenCalledWith('npx c3spec workspace update', {
      stdio: 'inherit',
      cwd: process.cwd(),
    });
    expect(execSync).not.toHaveBeenCalledWith('npx c3spec update', expect.anything());
  });
  ...
  it('no-op inside a workspace should warn when workspace skills drift', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('c3spec workspace update'));
  });
  ...
  it('core preset should preserve delivery setting', async () => {
    ...
    expect(config.workflows).toEqual(['explore', 'sync', 'archive']);
